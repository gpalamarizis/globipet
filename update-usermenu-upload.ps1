...
Set-Content -Path (Join-Path $root "apps\web\src\components\layout\MainLayout.tsx") -Value $f1 -Encoding UTF8 -NoNewline
...
'@
$dir = Split-Path (Join-Path $root "apps\web\src\lib\api.ts")
New-Item -ItemType Directory -Force -Path $dir | Out-Null
Set-Content -Path (Join-Path $root "apps\web\src\lib\api.ts") -Value $f2 -Encoding UTF8 -NoNewline
Write-Host "OK: apps\web\src\lib\api.ts"
