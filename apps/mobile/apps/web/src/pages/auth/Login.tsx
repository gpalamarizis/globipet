...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    catch (err: any) { toast.error(err?.response?.data?.message || err.message || t('authExtraLogin.invalidCredentials')) }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 via-white to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 p-4 relative">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end">
              <Link to="/forgot-password" className="text-xs text-brand-900 hover:underline">{t('authExtraLogin.forgotPassword')}</Link>
            </div>
        ...
        <p className="text-center text-sm text-gray-500 mt-4">
          {t('authExtraLogin.noAccount')} <Link to="/register" className="text-brand-900 font-medium hover:underline">{t('auth.register')}</Link>
        </p>
      </motion.div>
    </div>
  )
