"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DEFAULT_COMMISSION_RATES = void 0;
exports.getCommissionRates = getCommissionRates;
exports.setCommissionRates = setCommissionRates;
exports.resolveCommissionCategory = resolveCommissionCategory;
exports.calculateCommission = calculateCommission;
var prisma_js_1 = require("./prisma.js");
var SETTING_KEY = 'commission_rates';
exports.DEFAULT_COMMISSION_RATES = {
    food: 10,
    toys: 12,
    accessories: 12,
    services_default: 15,
    veterinary: 15,
    grooming: 15,
    training: 15,
    hosting: 15,
    walking: 15,
    pet_taxi: 15,
    photography: 15,
    pharmacy: 10,
    telehealth: 18,
};
var cache = null;
/** Reads commission rates from AppSetting (JSON blob), merged over defaults. Cached 30s in-process. */
function getCommissionRates() {
    return __awaiter(this, void 0, void 0, function () {
        var setting, stored, merged;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    if (cache && cache.expires > Date.now())
                        return [2 /*return*/, cache.rates];
                    return [4 /*yield*/, prisma_js_1.default.appSetting.findUnique({ where: { key: SETTING_KEY } })];
                case 1:
                    setting = _a.sent();
                    stored = {};
                    if (setting) {
                        try {
                            stored = JSON.parse(setting.value);
                        }
                        catch (_b) {
                            stored = {};
                        }
                    }
                    merged = __assign(__assign({}, exports.DEFAULT_COMMISSION_RATES), stored);
                    cache = { rates: merged, expires: Date.now() + 30000 };
                    return [2 /*return*/, merged];
            }
        });
    });
}
function setCommissionRates(partial) {
    return __awaiter(this, void 0, void 0, function () {
        var current, merged;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, getCommissionRates()];
                case 1:
                    current = _a.sent();
                    merged = __assign(__assign({}, current), partial);
                    return [4 /*yield*/, prisma_js_1.default.appSetting.upsert({
                            where: { key: SETTING_KEY },
                            update: { value: JSON.stringify(merged) },
                            create: { key: SETTING_KEY, value: JSON.stringify(merged) },
                        })];
                case 2:
                    _a.sent();
                    cache = null;
                    return [2 /*return*/, merged];
            }
        });
    });
}
/** Maps a product category or service_type string to a commission category key. */
function resolveCommissionCategory(input) {
    if (!input)
        return 'services_default';
    var key = input.toLowerCase();
    if (key === 'pet_sitting' || key === 'boarding')
        return 'hosting';
    if (exports.DEFAULT_COMMISSION_RATES[key] !== undefined)
        return key;
    return 'services_default';
}
/** Computes platform fee / provider payout split for a given amount and category. Rounds to 2 decimals. */
function calculateCommission(amount, category) {
    return __awaiter(this, void 0, void 0, function () {
        var rates, resolved, rate, platformFee, providerPayout;
        var _a;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0: return [4 /*yield*/, getCommissionRates()];
                case 1:
                    rates = _b.sent();
                    resolved = resolveCommissionCategory(category);
                    rate = (_a = rates[resolved]) !== null && _a !== void 0 ? _a : exports.DEFAULT_COMMISSION_RATES.services_default;
                    platformFee = Math.round(amount * (rate / 100) * 100) / 100;
                    providerPayout = Math.round((amount - platformFee) * 100) / 100;
                    return [2 /*return*/, { rate: rate, platformFee: platformFee, providerPayout: providerPayout }];
            }
        });
    });
}
