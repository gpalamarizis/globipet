...
function InsuranceTab() {
  ...
  const handleBulkImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (errors?.length > 0) toast.error(`⚠️ ${errors.length} σφάλματα`)
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <label className={`btn-secondary flex items-center gap-2 text-sm cursor-pointer ${importing ? 'opacity-50' : ''}`}>
            <Upload size={15}/> {importing ? 'Εισαγωγή...' : 'Bulk Import'}
      ...
      {isLoading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <button onClick={() => { setEditingProvider(null); setShowProviderModal(true) }} className="btn-primary inline-flex items-center gap-2 mt-2">
            ...
            <div key={provider.id} className="card overflow-hidden">
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => { setEditingPlan({ provider_id: provider.id }); setShowPlanModal(true) }}
      ...
      <InsuranceProviderModal open={showProviderModal} onClose={() => { setShowProviderModal(false); setEditingProvider(null) }}
      ...
      <InsurancePlanModal open={showPlanModal} onClose={() => { setShowPlanModal(false); setEditingPlan(null) }}
...
function InsuranceProviderModal({ open, onClose, editing, onSaved }: any) {
  const [form, setForm] = useState<any>({})
  useEffect(() => {
    if (open) setForm(editing ? { ...editing } : { name: '', name_el: '', website: '', phone: '', email: '', description: '', is_active: true })
  }, [open, editing])
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-modal max-w-md w-full max-h-[85vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!form.is_active} onChange={e=>setForm({...form,is_active:e.target.checked})}/> Ενεργή</label>
        </div>
...
function InsurancePlanModal({ open, onClose, editing, onSaved }: any) {
  const [form, setForm] = useState<any>({})
  ...
  const saveMutation = useMutation({
    mutationFn: () => editing?.id ? api.patch(`/admin/insurance/plans/${editing.id}`, form) : api.post('/admin/insurance/plans', form),
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-modal max-w-lg w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={e=>e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs font-medium text-gray-500 mb-1 block">Όνομα (EN) *</label><input className="input" value={form.name||''} onChange={e=>setForm({...form,name:e.target.value})}/></div>
            <div><label className="text-xs font-medium text-gray-500 mb-1 block">Όνομα (ΕΛ)</label><input className="input" value={form.name_el||''} onChange={e=>setForm({...form,name_el:e.target.value})}/></div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            ...
            <div><label className="text-xs font-medium text-gray-500 mb-1 block">€/μήνα *</label><input type="number" className="input" value={form.price_monthly||''} onChange={e=>setForm({...form,price_monthly:parseFloat(e.target.value)})}/></div>
            <div><label className="text-xs font-medium text-gray-500 mb-1 block">€/χρόνο</label><input type="number" className="input" value={form.price_annual||''} onChange={e=>setForm({...form,price_annual:parseFloat(e.target.value)||null})}/></div>
          </div>
          <div>
            <label className="text-xs font-medium text-gray-500 mb-2 block">Κάλυψη</label>
            <div className="flex flex-wrap gap-3">
              {[['covers_accidents','Ατυχήματα'],['covers_illness','Ασθένεια'],['covers_surgery','Χειρουργείο'],['covers_dental','Οδοντιατρείο'],['covers_preventive','Πρόληψη'],['covers_liability','Αστ. ευθύνη'],['covers_death','Θάνατος']].map(([key,label])=>(
                <label key={key} className="flex items-center gap-1.5 text-sm"><input type="checkbox" checked={!!form[key]} onChange={e=>setForm({...form,[key]:e.target.checked})}/> {label}</label>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-3">
              {[['dog','🐕 Σκύλος'],['cat','🐈 Γάτα'],['rabbit','🐇 Κουνέλι'],['bird','🦜 Πτηνό']].map(([type,label])=>(
                <label key={type} className="flex items-center gap-1.5 text-sm"><input type="checkbox" checked={(form.pet_types||[]).includes(type)} onChange={()=>togglePetType(type)}/> {label}</label>
              ))}
            </div>
          </div>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard icon={PawPrint}    label="Κατοικίδια"  value={stats?.pets ?? '—'}     change={8}   color="bg-orange-500" onClick={() => setActiveTab('pets' as Tab)} />
            ...
            <StatCard icon={TrendingUp}  label="Έσοδα"       value={`€${stats?.revenue ?? '0'}`} change={15} color="bg-purple-500" onClick={() => setActiveTab('revenue' as Tab)} />
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
