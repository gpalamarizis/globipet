...
export default function Register() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', password: '', role: 'user' as any })
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 via-white to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Δημιουργία λογαριασμού</h1>
          <p className="text-gray-500 text-sm mt-1">Γίνετε μέλος της κοινότητάς μας</p>
        </div>
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 block">Ονοματεπώνυμο</label>
              <input className="input" placeholder="Γιάννης Παπαδόπουλος" value={form.full_name} onChange={e => setForm(f => ({...f, full_name: e.target.value}))} required />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} className="input pr-10" placeholder="Τουλάχιστον 8 χαρακτήρες" value={form.password} onChange={e => setForm(f => ({...f, password: e.target.value}))} required minLength={8} />
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5 block">Τύπος λογαριασμού</label>
              <div className="grid grid-cols-2 gap-2">
                {[{v:'user',l:'Ιδιοκτήτης',e:'🐾',d:'Έχω κατοικίδιο'},{v:'service_provider',l:'Πάροχος',e:'🩺',d:'Παρέχω υπηρεσίες'}].map(opt => (
                  <button key={opt.v} type="button" onClick={() => setForm(f => ({...f, role: opt.v}))}
                    className={`p-3 rounded-xl border text-left transition-all ${form.role===opt.v ? 'border-brand-900 bg-brand-50 dark:bg-brand-900/20' : 'border-gray-200 dark:border-gray-700 hover:border-brand-300'}`}>
                    <span className="text-xl block mb-1">{opt.e}</span>
                    <span className="text-sm font-semibold text-gray-900 dark:text-white block">{opt.l}</span>
                    <span className="text-xs text-gray-500">{opt.d}</span>
                  </button>
                ))}
              </div>
            </div>
            <button type="submit" disabled={isLoading} className="btn-primary w-full">{isLoading ? 'Εγγραφή...' : 'Δημιουργία λογαριασμού'}</button>
          </form>

          <p className="text-xs text-gray-400 text-center mt-4">
            Αποδέχεστε τους <a href="#" className="text-brand-900 hover:underline">Όρους Χρήσης</a> και την <a href="#" className="text-brand-900 hover:underline">Πολιτική Απορρήτου</a>
          </p>
        </div>

        <p className="text-center text-sm text-gray-500 mt-4">
          Έχετε ήδη λογαριασμό; <Link to="/login" className="text-brand-900 font-medium hover:underline">Σύνδεση</Link>
        </p>
      </motion.div>
    </div>
  )
