...
function Register() {
    ...
    var _f = (0, react_1.useState)('user'), role = _f[0], setRole = _f[1];
    var _g = (0, react_1.useState)(false), showPass = _g[0], setShowPass = _g[1];
            ...
            switch (_c.label) {
                case 0:
                    ...
                    if (password !== confirmPassword) {
                        react_hot_toast_1.default.error(t('authExtraLogin.passwordMismatch'));
                        return [2 /*return*/];
                    }
                    _c.label = 1;
                case 1:
                    _c.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, register({ full_name: fullName, email: email, password: password, role: role })];
                case 2:
                    _c.sent();
                    navigate('/');
                    return [3 /*break*/, 4];
    ...
    return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 via-white to-blue-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 p-4 relative">
      ...
      <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-display font-bold text-gray-900 dark:text-white">{t('authExtraLogin.welcomeRegisterTitle')}</h1>
          <p className="text-gray-500 text-sm mt-1">{t('authExtraLogin.welcomeRegisterSubtitle')}</p>
        </div>
        ...
        <div className="card p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              ...
              <input type="text" className="input" value={fullName} onChange={function (e) { return setFullName(e.target.value); }} required/>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="relative">
                <input type={showPass ? 'text' : 'password'} className="input pr-10" placeholder="••••••••" value={password} onChange={function (e) { return setPassword(e.target.value); }} required minLength={6}/>
            ...
            <div>
              ...
              <input type={showPass ? 'text' : 'password'} className="input" placeholder="••••••••" value={confirmPassword} onChange={function (e) { return setConfirmPassword(e.target.value); }} required/>
            </div>
            <div>
        ...
        ].map(function (opt) { return (<button key={opt.value} type="button" onClick={function () { return setRole(opt.value); }} className={"p-2 rounded-xl border text-xs font-medium transition-all ".concat(role === opt.value
            ...
            <button type="submit" disabled={isLoading} className="btn-primary w-full mt-4">
              {isLoading ? t('authExtraLogin.registering') : t('auth.register')}
            </button>
          </form>
        </div>

        <p className="text-center text-sm text-gray-500 mt-4">
          {t('authExtraLogin.hasAccount')} <react_router_dom_1.Link to="/login" className="text-brand-900 font-medium hover:underline">{t('auth.login')}</react_router_dom_1.Link>
        </p>
